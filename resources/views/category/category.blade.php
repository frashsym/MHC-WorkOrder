                        ...
                        <div class="fixed inset-0 z-30 overflow-y-auto">
                            <div class="flex min-h-full items-center justify-center p-4">
                                <div x-show="modalOpen" x-transition @click.outside="modalOpen = false"
                                    ...
                                    <form :action="isEditMode ? '/category/' + formData.id : '{{ route('category.store') }}'" method="POST">
                                        @csrf
                                        <template x-if="isEditMode">
                                            <input type="hidden" name="_method" value="PUT">
                                        </template>
                                        <h2 class="text-lg font-semibold mb-4" x-text="isEditMode ? 'Edit Kategori' : 'Tambah Kategori'"></h2>
                                        <div class="mb-4">
                                ...
                                @foreach ($categories as $index => $category)
                                    <tr class="hover:bg-gray-50 dark:hover:bg-gray-800">
                                        ...
                                        <td class="px-4 py-2 space-x-2">
                                            <button @click="openEdit({ id: {{ $category->id }}, name: '{{ $category->name }}', department_id: {{ $category->department_id }} })"
