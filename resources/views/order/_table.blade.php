<div class="w-full overflow-x-auto">
    <table
        class="min-w-[800px] md:min-w-full border border-gray-300 dark:border-gray-700 divide-y divide-gray-300 dark:divide-gray-700">
        <thead class="bg-gray-100 dark:bg-gray-700 text-gray-700 dark:text-gray-200 text-sm">
            <tr>
                ...
                <th class="px-4 py-2 text-left whitespace-nowrap">Nomor Surat</th>
                <th class="px-4 py-2 text-left whitespace-nowrap">Tanggal</th>
                <th class="px-4 py-2 text-left whitespace-nowrap">Departemen</th>
                <th class="px-4 py-2 text-left whitespace-nowrap">Solver</th>
                <th class="px-4 py-2 text-left whitespace-nowrap">Kategori</th>
                <th class="px-4 py-2 text-left whitespace-nowrap">Aksi</th>
            </tr>
        </thead>
        <tbody class="divide-y divide-gray-300 dark:divide-gray-700 text-gray-800 dark:text-white text-sm">
            @forelse ($orders as $key => $order)
                <tr class="hover:bg-gray-50 dark:hover:bg-gray-800">
                    ...
                    <td class="px-4 py-2 whitespace-nowrap">{{ $order->title }}</td>
                    <td class="px-4 py-2 whitespace-nowrap">{{ $order->letter_number }}</td>
                    <td class="px-4 py-2 whitespace-nowrap">{{ $order->create_date ?? '-' }}</td>
                    <td class="px-4 py-2 whitespace-nowrap">{{ $order->department->name ?? '-' }}</td>
                    <td class="px-4 py-2 whitespace-nowrap">{{ $order->picUser->name ?? '-' }}</td>
                    <td class="px-4 py-2 whitespace-nowrap">{{ $order->category->name ?? '-' }}</td>
